...
const registerUser = async (payload) => {
    ...
    try {
        ...
        let userInsertQuery = `INSERT INTO users 
        ...
        let insertUserSecretsQuery = `INSERT INTO user_secrets (USER_ID,PASSKEY) VALUES($1,$2)`
        let userSecretValues = [user.rows[0].user_id, hashedPassword];
        await client.query(insertUserSecretsQuery, userSecretValues)
        if (location) {
            let insertUserAddressQuery = `INSERT INTO user_address (USER_ID,LOCATION) VALUES($1,$2)`
            let userAddressValues = [user.rows[0].user_id, location];
            await client.query(insertUserAddressQuery, userAddressValues)
        }
        //roles
        let insertUserRole = `INSERT INTO user_roles (USER_ID,ROLE_TYPE) VALUES($1,$2)`
...
const createUser = async (payload) => {
    ...
    let insertUser = `INSERT INTO users(USERNAME,FIRSTNAME,LASTNAME,GENDER) 
...
const validatedPassword = async (password, userId) => {
    let userSecretQuery = `SELECT * FROM user_secrets WHERE user_id= '${userId}'`
